...
maturity.ECB = c(0.5, seq(1, 30, by=1))

NSParameters <- Nelson.Siegel(rate = rate.ECB, maturity = maturity.ECB )
y1 <- NSrates(NSParameters[5, ], maturity.ECB)
plot(maturity.ECB, rate.ECB[5,], main="Fitting Nelson-Siegel yield curve", type="o")
lines(maturity.ECB, y1, col=2)
legend("topleft",legend=c("observed yield curve","fitted yield curve"),
       col=c(1,2),lty=1)
